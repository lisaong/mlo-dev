...
def naive_bmm_kernel(t0_ptr, t1_ptr, t2_ptr, n0, n1, n2, stride_t0n0,
                     stride_t0n1, stride_t1n0, stride_t1n2, stride_t2n2,
                     stride_t2n1, BLOCK_SIZE_N0: tl.constexpr,
    ...
    t1_ptrs = t1_ptr + ((offs_t1n0[:, None] * stride_t1n0) +
                        (offs_n2[None, :] * stride_t1n2))

    t2_ptrs = t2_ptr + ((offs_n2[:, None] * stride_t2n2) +
                        (offs_t2n1[None, :] * stride_t2n1))

    # Iterate to compute a block of the t0 matrix along the N2 dimension
    # Accumulate into a [BLOCK_SIZE_N0, BLOCK_SIZE_N1] block of fp32s
    accumulator = tl.zeros((BLOCK_SIZE_N0, BLOCK_SIZE_N1), dtype=tl.float32)
    for k in range(0, n2, BLOCK_SIZE_N2):
        # Load a block of T1 and T2
        # t1_block is [BLOCK_SIZE_N0, BLOCK_SIZE_N2]
        # t2_block is [BLOCK_SIZE_N2, BLOCK_SIZE_N1]
        # t1_block = t1.load(t1_ptrs)
        pass
...
def naive_bmm(t0, t1, t2, BLOCK_SIZE_K=32):
    ...
    grid = lambda META: (triton.cdiv(n0, META['BLOCK_SIZE_N0']) * triton.cdiv(
        n1, META['BLOCK_SIZE_N1']), )
    naive_bmm_kernel[grid](t0,
                           t1,
                           t2,
                           n0,
                           n1,
...
# %%
def bmm_reference(t0: torch.tensor, t1: torch.tensor,
