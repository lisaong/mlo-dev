...
__global__ void bandedMatMul_copysync(int n0, int n1, int n2, float *t0,
...
void run(int deviceId, Strategy strategy) {
  ...
  case Strategy::SynchronousCopy:
    ...
    bandedMatMul_copysync<<<blocks, threads, smemSize>>>(n0, n1, n2, T0.data,
                                                         T1.data, T2.data);
    break;
  default:
      ...
      try {
        ...
        while (elapsedTimeMilliseconds < kTimelimit) {
          ...
          case Strategy::SynchronousCopy:
            // shared memory: [t0 sub-matrix, t1 sub-matrix]
            smemSize = threads.x * threads.y * sizeof(float) * 2;
            bandedMatMul_copysync<<<blocks, threads, smemSize>>>(
...
int main(int argc, const char **argv) {
  int deviceId;
  Strategy strategy = Strategy::SynchronousCopy;
